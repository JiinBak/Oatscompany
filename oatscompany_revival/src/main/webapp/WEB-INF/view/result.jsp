...
d3.csv("https://s3.ap-northeast-2.amazonaws.com/oatsdata/originaldata/number.csv", function(d) {
	return {
		locale : d.locale,
		produce : d.produce,
		number : +d.number,
...
<script type="text/javascript">
    ...
    var x = d3.scaleOrdinal()
            .rangeRound([0, chartWidth]) // 왼 -> 오
            .domain(produce)
    /* y축의 범위 설정 */
    var y = d3.scaleLinear()
            .rangeRound([chartHeight, 0]) // 위 -> 아래
    ...
    item.enter()
        ...
        /* y(d.cpu_user); --> cpu_user 값을 scale에 맞게 비율 변환*/
            .attr("y", function(d) {
                //return d.cpu_user <- 비율 적용 전
                return y(d.cpu_user)
            })
            .attr("height", function(d, i) {
                //return chartHeight - d.cpu_user <- 비율 적용 전
                return chartHeight - y(d.cpu_user);
            })
    /*막대에 수치표시*/
    item.enter()
            .append("text")
